/**
 * Created by SHINING on 2017/3/11.
 * 应用程序的启动（入口）文件
 */
...
//设置cookies，后面的所有userInfo都是这里的cookies传过去的
app.use(function (req, res, next) {
    req.cookies = new Cookies(req, res);

    //解析登录用户的cookies信息
    req.userInfo = {};
    if(req.cookies.get('userInfo')){   //类型是字符串
        try{
            req.userInfo = JSON.parse(req.cookies.get('userInfo'));    //转化为JSON
...
/*
由用户发送http请求 -> url -> 解析路由 -> 找到匹配的规则 ->执行指定的绑定函数，返回对应内容至用户
