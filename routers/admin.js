...
* 分类首页
* /category : 表示url的访问方式
* admin/category_index : 表示html的所在路径，默认是html文件，不用加后缀
* render 可传入数据
* */
    ...
    Category.count().then(function (count) {
        ...
        * */
        Category.find().sort({_id: -1}).limit(limit).skip(skip).then(function (categories) {
            res.render('admin/category_index',{//渲染html模版
                userInfo: req.userInfo,
...
router.get('/category/edit',function (req, res) {
    //获取要修改的分类信息，并且用表单的形式展现出来，地址中问号后面的id值
    var id = req.query.id || '';
...
router.post('/category/edit',function (req, res) {
    ...
    }).then(function (sameCategory) { //return回来的数据在这
        if(sameCategory){
            res.render('admin/error',{
                userInfo: req.userInfo,
            ...
            return Category.update({  //更新数据库内容
                _id: id
            },{
                name:name
            });
...
router.get('/content',function (req, res) {
    ...
    Content.count().then(function (count) {
        ...
        Content.find().limit(limit).skip(skip).populate(['category','user']).sort({addTime: -1}).then(function (contents) {
            res.render('admin/content_index',{//渲染html模版
                userInfo: req.userInfo,
                contents: contents,
                count: count,
                pages: pages,
